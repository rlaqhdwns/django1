#views.py : View의 기능을 하는 클래스/함수를 정의하는 파이썬 파일

#render (함수) : 웹 클라이언트에게 HTML파일을 넘ㅇ겨줄때 사용하는 함수
from django.shortcuts import render


#뷰 함수 정의 시 첫번째 매개변수를 requeset로 설정해야함
#request : 사용자의 요청 정보, <form>으로 넘겨준 데이터, 로그인 정보, 세션정보,
#          요청방식(GET,POST) 등이 저장됨

#HTML 파일을 전송하는 메인페이지
def index(request):
    #render(request(매개변수), 클라이언트에게 줄 HTMl 파일 경로, HTML에 전달할 값)
    return render(request, 'bookmark/index.html',{'a':"hello",
                                                  'b':[1,2,3,4,'django']})
#Bookmark 모델클래스 임포트
...
def booklist(request):
    ...
    #모델클래스.objects.all() : 데이터베이스에 해당 모델클래스로 저장된 모든 객체를 추출 (list형태
    #모델클래스.objects.filter() : 데이터베이스에 저장된 특정조건을 만족하는 모든객체를 리스트형태로 추출
    #모델클래스.objects.exclude() : 데이터베이스에 특정조건을 만족하지 않는 모든객체를 리스트형태로 추출
...
def getbook(request, bookid):
    ...
    #=>뷰함수의 매개변수를 늘림 or <form>로 넘어온 데이터 처리
