...
from django.db import models #models : 모델클래스를 정의할때 사용하는 클래스가 정의된 파이썬 파일

# 모델 클래스 개발 및 반영 순서
#1) 모델 클래스 정의시, models.Model 클래스 상속받아 모델클래스를 정의
#2) 생성한 모델 클래스를 manage.py에 makemigrations 명령으로 migration파일 생성
#3) manage.py에 migrate 명령으로 데이터 베이스에 실질적인 저장공간 생성
...
class Bookmark(models.Model):
    ...
    #CharField : 글자 수 제한이 있는 문자열을 저장하는 공간을 정의
    #max_length : 최대 저장할 수 잇는 글자 수
