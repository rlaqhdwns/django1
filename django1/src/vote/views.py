...
#reverse(별칭문자열, args=(매개변수값)) : 별치기반으로 URL주소 매칭
#HttpResponseRedirect(url주소문자열) : 해당 URL주소를 클라이언트에게 전달

#get_object_or_404 : 특정한 모델클래스에 id값을 조건으로 검색해 객체 추출
#단, 객체가 존재하지 않는 경우에 웹 클라이언트의 요청이 잘못된 것으로 더 이상 뷰함수가 진행되지 않고 404에러페이지를 
#클라이언트에게 전달
...
def vote(request):
    ...
    #request.method : 사용자의 요청방식이 문자열형태로 저장된 변수 대소문자 구분이 있으므로 비교연산자 사용시"GET", "POST"로 비교
    if request.method == "POST":
        #request.POST : 클라이언트가 POST 방식으로 요청할 때 넘어온 데이터가 저장된변수
        #request.GET : 클라이언트가 GET 방식으로 요청할 때 넘어온 데이터가 저장된변수
...
def result(request, q_id):
    ...
    return render(request, 'vote/result.html', 
                  {'q': q})
#데코레이터 : URLConf를 통해 View함수가 호출 될 떄, 뷰가 실행되기 전
#         먼저 실행되는 함수.
#뷰함수에만 데코레이터를 적용할 수 있음
#뷰 클래스는 XXXMinxin 클래스를 상속받아 데코레이터로 처리
#데코레이터 적용 방식
#@데코레이터 함수 이름
#def 뷰함수(request):
...
#인 경우, 웹프로젝트에 지정된 로그인 URL로 넘어가는 데코레이터 함수

#로그인 URL 지정 방법
#setting.py -> LOGIN_URL 변수에 저장
...
def qregister(request):
    ...
    #사용자 요청이 POST방식일 떄의 처리
        ...
        if f.is_valid():
            ...
            #폼객체 기반으로 (QuestionForm) 기반으로 모델객체 (Question)객체 생성
            #모델폼객체.save() : 연동된 모델클래스의 객체로 변환 후, 데이터베이스에 저장
            ...
            q.data = datetime.now() #현재 컴퓨터의 날짜/시간정보를 date변수에 저장
...
def qupdata(request, q_id):
    ...
    if request.method == 'GET':
        ...
        #qregister.html과 유사하게 HTML코드가 구현되기 때문에 기존에 만들어진 HTMl파일을 재사용.
        #form태그의 action속성을 빈칸으로 처리했기 때문에, qregister 뷰에서 qregister.html를 쓴 것과, Qupdate 뷰에서 qregister.html를
...
def qdelete(request, q_id):
    ...
    #해당 객체를 데이터베이스에서 삭제함. 삭제한 객체에 저잔된 변수값은
    #사용할 수 있음
    q.delete()
...
def cupdata(request, c_id):
    ...
    if request.method == "POST":
        ...
        if f.is_valid():
            #데이터베이스에 수정사항반영
            f.save() #c변수와 f.save()함수에서 주는 Choice객체가 동일하기때문에 변수에 저장하지않음
            #다른 URL을 사용자에게 전송
            return HttpResponseRedirect( reverse('cote:detail',
                                                 args=(c.q.id,) ) )#튜플인식을위해서 마지막에,을 넣음
#Choice 객체 삭제
@login_required
def cdelete(request, c_id):
    c = get_object_or_404(Choice, id = c_id)
    c.delete()#데이터베잉스에서 Choice객체 삭제, 뷰함수 내에서는 변수값을 사용할 수 있음
    return HttpResponseRedirect(reverse( 'vote:detail',
                                         args=(c.q.id,)))
